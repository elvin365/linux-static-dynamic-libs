...
char* parse_args(int argc, char argv[][1024], conf_t* conf)
{
...
int main1()
{
	...
	conf_t conf;
	//printf("\nHere!\n");
	char argv1[100][1024]={0};
	int argc1=0;
	
	FILE* file=NULL;
	file=fopen("arguments.txt","r");
	if(file!=NULL)
	{	
		//int i=0;
		char buf[1024]={0};
		...
		{
	 		//fgets(argv1[i],500,file);
			buf[strlen(buf)-1]='\0';
			//printf("this is buf %s yes",buf);
			sprintf(argv1[argc1],"%s",buf);
			//printf("%s is argv1\n",argv1[argc1]);
			//i++;
			argc1++;
	...
	//printf("\n%d argc\n",argc1);
	char* conf_err = parse_args(argc1, argv1, &conf);
